...
require_relative '../data_stores/central_hash_example'
require_relative '../data_stores/concurrent_ruby_hash'
require_relative '../data_stores/mmap_store'
...
# Simple, label-less counters scenario
NUM_COUNTERS = 100
NUM_HISTOGRAMS = 0
MIN_LABELS = 0
MAX_LABELS = 0

# "Realistic", mixed counters and histograms with labels scenario
# NUM_COUNTERS = 80
# NUM_HISTOGRAMS = 20
# MIN_LABELS = 0
# MAX_LABELS = 4
...
TMPFS_DIR = "/tmpfs/prometheus_benchmark"
REDIS_CONNECTION_POOL = ConnectionPool.new(size: 20, timeout: 5) { Redis.new.tap{|r| r.select(13) } }
...
STORES = [
  ...
  {
    store: Prometheus::Client::DataStores::MmapStore.new(dir: TMP_DIR),
    name: "MmapStore (SSD)",
    before: -> (s) { cleanup_dir(TMP_DIR) },
  },
  {
    store: Prometheus::Client::DataStores::MmapStore.new(dir: TMPFS_DIR),
    name: "MmapStore (TMPFS)",
    before: -> (s) { cleanup_dir(TMPFS_DIR) },
  },
  {
    store: Prometheus::Client::DataStores::DirectFileStore.new(dir: TMP_DIR),
    name: "DirectFileStore (SSD)",
    before: -> (s) { cleanup_dir(TMP_DIR) },
  },
  {
    store: Prometheus::Client::DataStores::DirectFileStore.new(dir: TMPFS_DIR),
    name: "DirectFileStore (TMPFS)",
    before: -> (s) { cleanup_dir(TMPFS_DIR) },
  },
  {
    store: Prometheus::Client::DataStores::CentralHashExample.new,
    before: -> (s) { s.internal_store.clear },
  },
  {
    store: Prometheus::Client::DataStores::Pstore.new(dir: TMP_DIR),
    name: "Pstore (SSD)",
    before: -> (s) { cleanup_dir(TMP_DIR) },
  },
  {
    store: Prometheus::Client::DataStores::Pstore.new(dir: TMPFS_DIR),
    name: "Pstore (TMPFS)",
    before: -> (s) { cleanup_dir(TMPFS_DIR) },
  },
  {
    store: Prometheus::Client::DataStores::Redis.new(connection_pool: REDIS_CONNECTION_POOL),
    before: -> (s) { cleanup_redis },
  },
]
...
def cleanup_redis
  REDIS_CONNECTION_POOL.with { |conn| conn.flushdb }
end
...
Benchmark.bm(45) do |bm|
  THREAD_COUNTS.each do |num_threads|
    ...
    STORES.each do |store_test|
      ...
      # Cleanup before test
      store_test[:before].call(store_test[:store]) if store_test[:before]
